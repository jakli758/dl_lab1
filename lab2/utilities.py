...
from tf_keras.layers import Input, Dense, BatchNormalization, Dropout, Reshape, Activation
...
def build_DNN(input_shape, n_hidden_layers, n_hidden_units, loss, act_fun='sigmoid', optimizer:str='sgd', learning_rate=0.01, 
            use_bn=False, use_dropout=False, use_custom_dropout=False, print_summary=False, use_variational_layer=False, print_args=False, kl_weight=None):
    """
    ...
    for _ in range(n_hidden_layers):
        if use_variational_layer:
            model.add(tfp.layers.DenseVariational(n_hidden_units, activation=act_fun, make_posterior_fn=posterior, make_prior_fn=prior, kl_weight=kl_weight))
        else:
            model.add(Dense(n_hidden_units, activation=act_fun, input_shape=(n_hidden_units,)))
        if use_bn:
...
def train_DNN(config, data, training_config):
    ...
    if training_config.get('class_weight'):
        model = model.fit(x=X_train, y=y_train, validation_data=(X_val, y_val), epochs=training_config['epochs'], batch_size=training_config['batch_size'], callbacks=[TuneReporterCallback()], class_weight=training_config['class_weight'])
    else:
        model = model.fit(x=X_train, y=y_train, validation_data=(X_val, y_val), epochs=training_config['epochs'], batch_size=training_config['batch_size'], callbacks=[TuneReporterCallback()])
    
    return model
    # --------------------------------------------

def train_DNN_tune(config, data, training_config):
    '''
    Train a DNN model based on the provided configuration and data. 
    This is use in the automatic hyperparameter search and follows the format that Ray Tune expects.
